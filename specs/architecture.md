...
- find_video intent (DD-24): "小雞老師找影片" — search layer goes through self-hosted **Invidious** (`InvidiousClient`, `/api/v1/search` region=TW), borrowing the same-host ytlite approach → **zero YouTube Data API quota**. Data API (`YoutubeVideoProvider.runQuery`, safeSearch=strict) is the fallback only when Invidious is unavailable. Playback is unchanged: real YouTube `videoId` rendered via IFrame inline player. Child safety = two gates: (1) curated channel library (`ChildChannelLibrary` active) always passes + stably sorts first; (2) non-curated channels filtered by Invidious channel-level `isFamilyFriendly` (YouTube familySafe microformat, 24h cache, parallel unique-channelId query) — unknown verdicts conservatively dropped (Invidious search has no safeSearch param). Source priority controlled by `INVIDIOUS_API_URL` env (default `http://localhost:1215` → same-host ytlite; empty string disables Invidious → Data API). **Shared Invidious layer (DD-31, supersedes DD-30)**: cecelearn does NOT bundle its own Invidious — it connects to a standalone **shared Invidious layer** (`/home/pkcs12/projects/invidious-shared`, owned by neither project: `db + companion + engine`, external docker network `invidious_shared_net`, host port `1215`). Both cecelearn (via host `localhost:1215`) and ytlite (via internal alias `invidious:3000` after joining `invidious_shared_net`) consume the same instance — connection strings unchanged on both sides. The shared layer was extracted from ytlite's compose with zero data-migration risk (Invidious postgres is pure cache: users/playlists/videos all empty). Consequences: (1) the shared Invidious layer must be up on the same host; if down, find_video fail-soft degrades to Data API (if keyed) or existing video bank, never crashes; (2) feed prewarm (`POST /api/a1/prewarm`, channelLatestVideos aggregation) requires Invidious **≥2026.06** (channel parser fix, see DD-29) — the shared layer pins `2026.06.15`; (3) backend startup runs a one-shot Invidious health probe (`InvidiousClient.ping()` → `/api/v1/stats`), logging OK or a WARN naming the dependency (never blocks boot). Shared layer lifecycle: `cd /home/pkcs12/projects/invidious-shared && docker compose up -d`. To make cecelearn fully self-contained, point `INVIDIOUS_API_URL` at a dedicated Invidious instance.
