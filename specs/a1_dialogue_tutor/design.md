...
- **DD-35**: 語音輸入「靜默寬限窗 + 片段累積」送出（DD-24）。根因：A1Page recognition.onresult 原本一收到 isFinal 就立即 lookupRef.current(transcript) 送出，但 Web Speech API 的 isFinal 是瀏覽器 VAD 在 ~0.8s 靜默後觸發——小朋友句中停頓思考會被誤判為「講完了」而提早送半句。修法：onresult 不再立即送，改累積 final 片段進 pendingTranscriptRef，每次新語音（interim 或 final）以 armCommitTimer() 重置 SILENCE_COMMIT_MS=2500ms 計時；唯有真的停夠久 commitPending() 才整段送出（句中停頓持續延後）。回音（isWithinSpeechGuard/isLikelySelfEcho）不洗掉已累積真實語音、也不延長窗；超長(>200字)保護直接送。所有停止/切換路徑（onerror 硬失敗、effect cleanup、overlay 互斥、toggleListening 手動停、handleVideoPlayingChange 影片暫停辨識）均呼叫 discardPending() 丟棄半累積緩衝與清計時器，避免停麥後殘段補送。沿用 DD-10 最小變更原則：辨識核心 VAD/重啟/Samsung 路徑不動，只改辨識結果下游的送出時機。Samsung manual mode 維持單發即送不受影響。
- **DD-36**: 語音「隨時中斷來傾聽」barge-in（DD-25）。需求修正重點：使用者要的不是「等講完再送」，而是「小雞老師朗讀中，小朋友能隨時插話打斷、小雞立刻停下來傾聽」。問題：原 echo 軟閘（isWithinSpeechGuard）在 TTS 播放中無差別丟棄所有辨識結果 → 小朋友根本無法插話。修法（A1Page onresult，沿用 DD-10 不重寫核心）：朗讀中收到辨識結果時，用 isLikelySelfEcho 區分「小雞自己的回音」（文字吻合最近朗讀→丟棄維持防迴圈）與「小朋友插話」（非回音→立刻 cancelSpeech() 停止朗讀、設 childHasFloor 取得發言權）。取得發言權後該段及後續語音照常累積（DD-24 寬限窗送出），echo 軟閘暫時略過（小雞已停、無自身回音可防）。三個協調點：(1) effect 內 suppressRestartOnBargeIn flag 讓「朗讀結束→重啟辨識」自我修復跳過這次，避免 abort 洗掉正在捕捉的插話；(2) childHasFloor 在 commitPending/discardPending 重置，下一輪重新評估 echo 軟閘不殘留；(3) 跟讀借用路徑（captureStateRef）不受影響。Samsung manual mode 不適用 barge-in（單發模式）。
- **DD-38**: 語音輸入「邊聽邊解讀，講完才回應」——單一原則收斂（取代 DD-35 純計時窗，亦廢止本 DD 早期的「分級窗 / 短指令 vs 唸長題雙模式 / 字數門檻」設計）。需求演進：使用者回報小孩唸題目沒那麼順——前端會自動送字（唸到一半被截斷送出）、把前半段內容歸零重唸。**關鍵心智校正（使用者口述）**：小孩唸習題通常不是一兩句，而是「一整段文章」；不管長短，AI 終究要解讀，所以應「不管聽到多少字，一律邊聽邊累積邊解讀，直到明確判斷發話者講完，才輪到 AI 回應」。據此把先前過度設計（READING_MODE_CHARS 字數門檻、雙模式、>200 字硬截斷、complete 一遇句尾就 1.2s 快送）全部移除，收斂為單一路徑。根因（A1Page onresult/commit）：(1) 舊 commitPending 純靜默計時（停夠久就送）不看內容，句中停頓被誤判講完送半句。(2) interim 只預覽不進 pending，session abort/重啟（TTS 後 restartSession、watchdog、Chrome 自發 onend）丟失 in-flight 半句。(3) `length>50` / `>200` 字截斷把長題中途送出。最終心智模型（使用者第三次校正定稿）：「話講多講少最終都要送 AI，那就邊送邊判斷」——前端只做一件事：偵測到停頓 1 秒就把累積的字送給 AI；前端**完全不猜內容**。判斷「講完了沒」100% 在 AI，而且 AI 要**老實準確**判斷、不偏向任何一邊：短指令（用蘋果造句）一聽就知道完了→回 true 快回應；半句頓一下→知道還有→回 false 繼續聽。修法（沿用 DD-10 不重寫辨識核心，只改結果下游）：① 半句保全——interimTranscriptRef 獨立保存未定稿 interim，onend promote 進 pending、commitPending 送出時併入。② 移除所有字數截斷（不分長短、無上限，一律邊聽邊累積邊回填輸入框，永不因累積過長中途送）。③ 單一閘門＝「發話者講完了沒」，判斷交給 AI（後端 utterance-complete）：每次新語音統一重排 SILENCE_MS=1000ms 靜默計時（**前端不再用 classifyUtterance 判內容微調延遲——該規則函式已整個刪除**），停夠久才 probeAndDecide 把累積的字送 AI 判「講完了沒」；complete→commitPending 送，not-complete→notDoneStreak 計數 + PATIENT_MS=1500ms 看孩子是否續講（有新語音歸零），連續 MAX_NOTDONE_STREAK=2 次「同段且無新語音」才兜底送（防少數誤判卡死），AI 不可用→FALLBACK_MS=6000ms 純計時兜底（唯一不經解讀者的路徑，fail-fast）。④ 後端 prompt 改為「老實準確判斷、不偏向」：語意完整的指令或問題（不論長短）→ true（短指令一聽就知道完了就回 true 讓老師快回應）；明顯還沒講完（懸著連接詞/虛詞、語意斷半句、唸長題中間句號只是段落中途）→ false；只在真的拿不準時才稍偏 false（多等不打斷），但不可把明顯講完的短指令也判沒講完。端點 UtteranceCompleteEngine（gemini-2.5-flash-lite + thinkingBudget:0、2s timeout、cache+inflight 去重、responseSchema {complete:boolean}）。Samsung 單發模式不適用。backend+frontend tsc EXIT=0。驗證：用蘋果造句/三乘七怎麼算→complete:true，唸題到一半/句尾懸連接詞→false，整段唸完→true（curl 五案全綠）。
- **DD-37**: 填空題判題改比對「等價答案清單」acceptableAnswers（DD-26）。問題：學科練習填空題判題（QuizPage.judge）原本只比 norm(given)===norm(answer) 或數值相等，answer 是純數值「3.2」→ 答「3.2公尺」字串不等、Number("3.2公尺")=NaN，兩路皆失敗被誤判錯。修法（順使用者設計判斷：標準答案既由出題 AI 生，就讓 AI 一併列出所有應判對寫法）：後端生題契約新增 acceptableAnswers:string[]——quizFramework GenItem 加欄、responseSchema 加 acceptableAnswers、buildPrompt 指示 fill 題列出帶單位/不帶單位/單位換算/中英單位/分數小數互換等所有等價寫法且 answer 須在內、genForKp 抽取去重併入 answer；quizBankProvider BankItem+QuizServeItem 加欄並 serve 帶出；quizGenProvider toServe 帶出；前端 client.ts QuizServeItem 鏡像加欄。判題：judge 比對池 = [answer, ...acceptableAnswers]，逐項 matchesOne（正規化字串相等 OR 兩邊數值相等），命中任一即對。安全性：只接受 AI 列出的真正等價寫法，錯單位（3.2公分）不在清單仍判錯。choice/make_word/read_aloud 不受影響。backend+frontend typecheck EXIT=0。
- **DD-39**: 影片自動接下一部（VideoPlayer 連續播放）。需求：使用者要求影片播完能自動接下一部。實作：VideoPlayer.tsx onStateChange 的 S.ENDED 分支，原本只 notify(false) 停下，改成「佇列裡還有下一支就 goRef.current(index+1) 自動載入續播（同播放窗、loadVideoById 不重打 API，隨後 PLAYING 事件再關麥克風），到佇列最後一部才 notify(false) 停下」。邊界（6–9 歲兒童 app 防無限 autoplay 黑洞）：只自動接「已露出的佇列」（通常 3–6 部），播完最後一部就停，不自動觸發 onLoadMore——載更多仍需手動按。技術細節：go 函式原為元件 body 的普通宣告，effect 內 closure 抓不到最新 closure，故新增 goRef（每 render goRef.current=go）供 onStateChange 呼叫。附帶 UI 微調：移除 footer card「小雞老師想一想…」字樣（setStatus('') 取代），思考中只靠 busy 轉圈呈現。驗證：frontend tsc -b EXIT=0。純前端，Vite HMR 自動套用不需重啟 backend。
- **DD-40**: 語畢送字慢（實機「起碼五秒」）根因與修法：interim 不觸發送出計時，枯等 Chrome 慢定稿。根因（system-first 診斷）：onresult 的判斷路徑（armCommitByContent/armSilence callback/probeAndDecide 比對）全部只看 pendingTranscriptRef，而 pendingTranscriptRef 只有 final 才寫入。interim（未定稿）階段送出計時器根本不啟動，要枯等 Chrome webkitSpeechRecognition 在 continuous=true 下的內部 endpointer 把 interim 轉成 final——這段延遲常 1.5–3s 且 JS 不可控。時間軸＝Chrome endpointer(1.5–3s 不可控) + SILENCE_MS(0.6s) + probe(~1.2s) ≈ 3.3–5s，正是「起碼五秒」。也解釋為何先前把 SILENCE_MS 1000→600 體感無感（削的是被大頭蓋住的小項）。修法（DD-38 延伸，符合「邊送邊判斷」）：新增 currentFullText() = pending + interim 合併文字，把四個判斷站點全部改成看它而非只看 final。interim 一停（SILENCE_MS 內無新語音）就直接拿合併文字問 AI，繞過 Chrome 慢定稿。語畢送出 ~3.3–5s → ~1.8–2.3s。取捨：極少數情況 interim 與最終 final 差一兩字（短指令影響微，AI 容錯）。驗證：frontend tsc -b EXIT=0。純前端 Vite HMR 自動套用不需重啟 backend。
- **DD-41**: DD-40 regression 修復：interim 納入判斷後「完全不送字」的計時迴圈死鎖。根因（liveness deadlock）：probeAndDecide 有兩個 early-return 會把送出計時迴圈整個丟掉而不重排——(1) 頂部 `if (probing) return`：interim 高頻觸發（onresult 每秒多次）的計時器若落在前一個 probe 的飛行視窗（~1.2s）內，被這裡靜默吞掉；(2) `.then` 裡 `if (current !== text) return`：probe 飛行中 interim 又變了（interim 本質持續變動，幾乎必發生），也靜默 return。兩條路徑都不重排計時器，於是孩子一停（不再有新 interim 觸發 armCommitByContent）就沒有任何計時器在排程→永不送出。為何 final-only 時無事：final 離散低頻，計時器撞進 probe 視窗機率極低；換 interim 後幾乎必中。修法：兩個 early-return 改成 armSilence(SILENCE_MS) 重排而非丟棄，讓迴圈活著，等下一次停頓用新文字再問。armSilence 會先 clearTimeout 舊計時器，不會疊加。驗證：frontend tsc -b EXIT=0。純前端 Vite HMR 自動套用不需重啟 backend。
- **DD-42**: 語音語畢等待不穩定修正：取消前端 6 秒長窗兜底與 `MAX_NOTDONE_STREAK` 自行送出。使用者明確要求「前端很快送到後端，後端 AI 決定什麼時候收尾執行反應」。修法：前端保留 `SILENCE_MS=600ms` 快速 probe，`PATIENT_MS` 降為 700ms；後端 `utterance-complete` 新增 `quietRepeatCount`，同段文字在無新語音後重問時把「安靜重試次數」交給後端判斷是否已足以 `complete=true`。latency 補正：快判 Gemini timeout 從 2s 降到 900ms；若同一段文字在無新增語音後已重問 1 次，A1 module 直接回 `complete=true`（provider 內也保留同門檻），即使 provider 未配置或上游不可用也不會讓前端只短間隔重試造成永不送字；這個保底在後端，不恢復前端 6 秒計時收尾。觀測：前端 console `A1Speech` 與後端 `UtteranceCompleteAPI` log 會輸出每次 probe elapsed。
