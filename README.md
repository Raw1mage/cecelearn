...
兒童安全採「**家長黑名單硬擋＋精選徽章提示**」：命中家長黑名單的頻道一律剔除（硬擋）；命中精選頻道庫的標 ⭐ 精選徽章。**精選只是徽章，不重排序**——刻意保留搜尋來源回來的相關度原序，避免任何搜尋的第一支永遠是訂閱頻道（之前把精選無條件排到最前，導致「不管搜什麼都只出現訂閱頻道」）。
...
**兒童知識型頻道庫**（`data/channels.json`，如樂樂TV、十萬個為什麼、成語任務）：命中庫內頻道的結果標 ⭐ 精選徽章（前端提示用），管理 `GET /api/a1/channels`、`POST /api/a1/channels {channelId,…}`。
