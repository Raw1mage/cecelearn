...
| **成語填字闖關** | 「玩成語填字」「來填字」「成語闖關」 | 演算法自動生成十字交叉成語盤，點備選字填空、填對揭曉釋義例句＋朗讀、過關進下一關（零後端成本） |
| **英文單字練習** | 「我要練習英文單字」「考我英文單字」 | 看圖→聽發音（TTS `en-US`）→手寫拼字的全螢幕卡片；手寫覆蓋度比對判對，答對灑花＋計分＋XP 升級，並播 ≤2 分鐘信任頻道**獎勵短片**（卡片適應頁寬、播放時收起寫字板留拼字格） |
| 遊戲語音啟動 | 上述測驗/遊戲皆可「用說的」開啟 | 所有遊戲統一走 game registry（單一來源）→ 語音 intent / 首頁入口鈕 / 全螢幕 overlay 一致；新遊戲加一筆 entry 即具備 |
...
兒童安全採三道閘：**家長黑名單硬擋** ＋ **信任頻道白名單優先過濾** ＋ ⭐ 精選徽章。命中黑名單的頻道一律剔除（硬擋）。信任頻道（`channels.json` active）過濾策略＝**白名單優先、空時才補**：搜尋結果裡只要有信任頻道的片，就**只服務那些、免逐片審直接給孩子看**；一支都沒命中才整批照原樣回（`safeSearch` + 黑名單仍在），不開天窗。這是對**已按相關度排好的結果做過濾**、非強制置頂——信任頻道的片本來就因相關才出現，故不重蹈「不管搜什麼第一支都是訂閱頻道」那個排序蓋相關度的老問題。

> **serve 路徑一律先搜新鮮網路結果**：yt-dlp 是零配額被動函式，沒有「省額度」的理由去吐庫內舊資料。每次都忠實反映當下的搜尋詞，不會「庫滿短路」、不會固化污染。

**影片庫純離線安全網**（`data/genbank.sqlite` 的 `gen_video` 表，取代舊 `videobank.json`）：搜回的結果仍分門別類**寫回庫**累積，但**只在「網路整個搜失敗」（yt-dlp 與 Data API 都拿不到任何結果）時才退而服務庫內既有**，讓對話不至於開天窗；正常情況永遠走新鮮搜尋。後台檢索 `GET /api/a1/videobank`。

**兒童知識型頻道庫**（`data/channels.json`，15 個 active：中文如樂樂TV、十萬個為什麼、成語任務；英文如 Alphablocks、Rock 'N Learn、Super Simple ABCs、Phonics Reading）：命中庫內 active 頻道的片參與上述白名單優先過濾並標 ⭐ 精選徽章；a6 英文獎勵短片也優先取這些信任英文頻道。管理 `GET /api/a1/channels`、`POST /api/a1/channels {channelId,…}`。
